...
public enum UserDAOImpl implements UserDAO {
    ...
    public User getByUsername(String username) {
        return QueryRunnerProxy.query(SQLs.USER_GET_BY_USERNAME, RsHandlers.USER, username);
    }

    @Override
    public int saveUser(User user) {
        return QueryRunnerProxy.update(SQLs.USER_SAVE_USER, user.getType(), user.getUsername(), user.getPassword());
    }

    @Override
    public int updateById(User user) {
        return QueryRunnerProxy.update(SQLs.USER_UPDATE_BY_ID, user.getPassword(), user.getId());
    }
}
